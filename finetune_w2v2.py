...
from transformers import AutoTokenizer, AutoModelForCausalLM, PretrainedConfig, Wav2Vec2Model,__version__
import time
...
class DataCollatorCTCWithPadding_Speed_Perturb:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # 在for循环中随机设置speed，每个音频都是随机速度，效果更好
...
def main():
    ...
    model.config.mask_time_prob = 0.65
    ...
    # state_dict = torch.load("/data2_from_58175/huggingface/models/wav2vec2-base/pytorch_model.bin",map_location="cpu")
    # print("reload")
    ...
    # aishell2_train_dataset = load_from_disk("/data2_from_58175/huggingface/datasets/aishell2/train")
    # aishell_train_dataset = load_from_disk("/data2_from_58175/huggingface/datasets/aishell/train")
    # ws_train_dataset = load_from_disk("/data2_from_58175/huggingface/datasets/ws_train")
    ...
    # aishell2_train_dataset = aishell2_train_dataset.remove_columns(ignored_columns).remove_columns(["id"])
    # aishell_train_dataset= aishell_train_dataset.remove_columns(ignored_columns).remove_columns(["id"])
    # ws_train_dataset = ws_train_dataset.remove_columns(ignored_columns).remove_columns(["seg_start","seg_end","id"])
    # for i in [train_dataset,aishell2_train_dataset,aishell_train_dataset]:
        # print(i)
    # train_dataset= datasets.concatenate_datasets([train_dataset,aishell2_train_dataset,aishell_train_dataset])
